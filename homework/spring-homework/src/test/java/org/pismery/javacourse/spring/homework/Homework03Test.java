...
class Homework03Test {
    ...
    public void batchInsertTest() {
        Homework03 homework03 = new Homework03();
        System.out.println("batchInsertTest: ");

        int endExclusive = 10000;
        try (Connection connection = Homework03.ConnectionManager.getConnectionFromDriver()) {
            LocalDateTime begin = LocalDateTime.now();
            List<List<Object>> insertDataList = IntStream.range(1, endExclusive)
                    .mapToObj(i -> Arrays.asList((Object) i, i, i))
                    .collect(Collectors.toList());
            homework03.executeBatch(connection, "insert into t(id,c,d) values (?,?,?)", insertDataList);

            System.out.println(String.format("Batch Insert Cause %s ms",Duration.between(begin,LocalDateTime.now()).toMillis()));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
    ...
    public void insertTest() {
        ...
        try (Connection connection = Homework03.ConnectionManager.getConnectionFromDriver()) {
            ...
            System.out.println(String.format("Insert Cause %s ms",Duration.between(begin,LocalDateTime.now()).toMillis()));
        } catch (SQLException e) {
            e.printStackTrace();
        }
    }
